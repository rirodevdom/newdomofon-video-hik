...
def patch_recorder(path: Path) -> None:
    ...
    text = replace_once(
        ...
        "recorder x264 startup",
    )
    text = replace_once(
        text,
        "    ...(nodeArchive ? ['-strftime', '1', '-strftime_mkdir', '1'] : ['-hls_delete_threshold', '2']),",
        "    ...(nodeArchive ? ['-strftime', '1', '-strftime_mkdir', '1'] : ['-hls_delete_threshold', String(Math.max(4, config.liveWindow))]),",
        "live delete threshold",
    )
    text = replace_once(
        text,
        "    const wanted = new Map<string, { device: HikvisionDeviceConfig; channel: HikvisionChannel }>();",
        "    const wanted = new Map<string, { device: HikvisionDeviceConfig; channel: HikvisionChannel; liveStreamId: string }>();",
        "wanted map type",
    )
    text = replace_once(
        text,
        "        wanted.set(channel.id, { device: item.config, channel });",
        "        const liveStreamId = selectLiveStreamId(channel, config.liveStreamPolicy);\n        wanted.set(channel.id, { device: item.config, channel, liveStreamId });",
        "wanted stream selection",
    )
    text = replace_once(
        text,
        "      if (!next || runtime.key !== recorderKey(next.device, next.channel)) {",
        "      if (!next || runtime.key !== recorderKey(next.device, next.channel, next.liveStreamId)) {",
        "recorder reconcile key",
    )
    text = replace_once(
        text,
        "      if (!this.recorders.has(channelId)) await this.start(next.device, next.channel);",
        "      if (!this.recorders.has(channelId)) await this.start(next.device, next.channel, next.liveStreamId);",
        "recorder start selected stream",
    )
    text = replace_once(
        text,
        ...
        "  private async start(device: HikvisionDeviceConfig, channel: HikvisionChannel, liveStreamId: string): Promise<void> {\n    const runtime: RuntimeRecorder = {\n      key: recorderKey(device, channel, liveStreamId),\n      device,\n      channel,\n      liveStreamId,",
        "recorder runtime selected stream",
    )
    text = replace_once(
        text,
        ...
        "    const candidates = liveRtspCandidates(runtime.device, runtime.channel, runtime.liveStreamId);\n    const input = candidates[runtime.candidateIndex % candidates.length]!;\n    const { cwd, args } = hlsArgs(runtime.device, runtime.channel, runtime.liveStreamId, input);",
        "recorder candidates selected stream",
    )
    text = replace_once(
        text,
        ...
        "      console.log(`[recorder:${runtime.channel.id}] started pid=${child.pid} stream=${runtime.liveStreamId} source=${redactRtsp(input)} archive=${runtime.channel.archive_storage}`);",
        "recorder startup diagnostics",
    )
    text = replace_once(
        text,
        "    const candidate = liveRtspCandidates(runtime.device, runtime.channel)[runtime.candidateIndex] || null;",
        "    const candidate = liveRtspCandidates(runtime.device, runtime.channel, runtime.liveStreamId)[runtime.candidateIndex] || null;",
        "recorder status selected stream",
    )
    path.write_text(text, encoding="utf-8")
...
def patch_archive_search(path: Path) -> None:
    text = path.read_text(encoding="utf-8")
    marker = "interface ArchiveSearchCacheEntry"
    if marker not in text:
        insertion = """
interface ArchiveSearchCacheEntry {
  startMs: number;
  endMs: number;
  expiresAt: number;
...
function rememberArchiveItems(device: HikvisionDeviceConfig, channel: HikvisionChannel, start: Date, end: Date, items: DeviceArchiveItem[]): void {
  ...
  next.push({
    ...
    endMs: end.getTime(),
    expiresAt: now + config.archiveRangeCacheMs,
