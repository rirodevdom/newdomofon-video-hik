...
SOURCE="$PROJECT_DIR/native-sdk/hik_sdk_worker.cpp"

fail() { echo "ERROR: $*" >&2; exit 1; }
[[ -f "$HEADER" ]] || fail "HCNetSDK header is not installed: $HEADER"
[[ -f "$SOURCE" ]] || fail "Native worker source is missing: $SOURCE"
...
echo "Rebuilding HCNetSDK worker from current project source"
g++ -std=c++17 -O2 -pthread \
  -I"$SDK_ROOT/include" \
  "$SOURCE" \
  -L"$LIB_DIR" -lhcnetsdk \
  -Wl,-rpath,"$LIB_DIR" \
  -o "$SDK_ROOT/bin/hik-sdk-worker.bin.new"
chmod 0755 "$SDK_ROOT/bin/hik-sdk-worker.bin.new"
mv -f "$SDK_ROOT/bin/hik-sdk-worker.bin.new" "$SDK_ROOT/bin/hik-sdk-worker.bin"
